...
class _LayerReplay:
    ...
    def _get_materialized_shared_kv(
        ...
        if self._materialized_shared_kv is None:
            self._materialized_shared_kv = (
                self.self_outer._materialize_shared_kv_states(
    ...
    def __call__(
        ...
        if self.current_per_layer_input_source is not None:
            if chunk_range is None:
                ...
                per_layer_input_local = self.self_outer._move_tensor(
                    self.current_per_layer_input_source[
...
class _StreamGemmaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        query_input = (
            hidden_states
            if hidden_states.shape[1] == chunk_len
            else hidden_states[:, start:end, :]
        )
        query_states = self.q_proj(query_input).view(
            batch_size, chunk_len, -1, self.head_dim
        )
        ...
        if (
            self._replay_key_states is not None
            and self._replay_value_states is not None
        ):
            ...
            key_states = self.k_proj(kv_hidden).view(
                batch_size, kv_len, -1, self.head_dim
            )
            value_states = (
            ...
            if need_full_store:
                if kv_start == 0:
                    ...
                    full_key_states = self.k_proj(full_kv_hidden).view(
                        batch_size, end, -1, self.head_dim
                    )
                    full_value_states = (
                        self.v_proj(full_kv_hidden).view(
                            batch_size, end, -1, self.head_dim
                        )
                        if self.v_proj is not None
...
class _StreamGemmaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if use_replay_cache:
            hidden_states_norm = self.base_layer.input_layernorm(
        ...
        hidden_states_chunk = self.base_layer.pre_feedforward_layernorm(
            hidden_states_chunk
        )
        hidden_states_chunk = self.base_layer.mlp(hidden_states_chunk)

        if getattr(self.base_layer, "enable_moe_block", False):
            hidden_states_1 = self.base_layer.post_feedforward_layernorm_1(
                hidden_states_chunk
            )
            hidden_states_flat = residual.reshape(-1, residual.shape[-1])
            _, top_k_weights, top_k_index = self.base_layer.router(hidden_states_flat)
            hidden_states_2 = self.base_layer.pre_feedforward_layernorm_2(
                hidden_states_flat
            )
            hidden_states_2 = self.base_layer.experts(
                hidden_states_2, top_k_index, top_k_weights
            )
            hidden_states_2 = hidden_states_2.reshape(residual.shape)
            hidden_states_2 = self.base_layer.post_feedforward_layernorm_2(
                hidden_states_2
            )
            hidden_states_chunk = hidden_states_1 + hidden_states_2

        hidden_states_chunk = self.base_layer.post_feedforward_layernorm(
            hidden_states_chunk
        )
        hidden_states_chunk = residual + hidden_states_chunk
        ...
        if getattr(self.base_layer, "hidden_size_per_layer_input", 0):
            ...
            if per_layer_input is None:
                raise RuntimeError(
                    "per_layer_input is required for chunked Gemma4 replay"
                )
            if per_layer_input.shape[1] != chunk_len:
                per_layer_input = per_layer_input[:, start:end, :]
            hidden_states_chunk = self.base_layer.per_layer_input_gate(
                hidden_states_chunk
            )
            hidden_states_chunk = self.base_layer.act_fn(hidden_states_chunk)
            hidden_states_chunk = hidden_states_chunk * per_layer_input
            hidden_states_chunk = self.base_layer.per_layer_projection(
                hidden_states_chunk
            )
            hidden_states_chunk = self.base_layer.post_per_layer_input_norm(
                hidden_states_chunk
            )
            hidden_states_chunk = residual + hidden_states_chunk
...
def _make_chunk_attention_mask(
    ...
    *,
    ...
    if raw_attention_mask is not None:
        pad = raw_attention_mask[:, kv_start:kv_end].to(
