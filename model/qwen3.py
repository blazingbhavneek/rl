...
class Qwen3Model(BaseModel):
    ...
    def _forward_prefix(
        ...
        cache_position = torch.arange(seq_len, device=input_ids.device, dtype=torch.long)
        try:
    ...
    def _forward_suffix(
        ...
        cache_position = (
            text_position_ids[0]
            if text_position_ids.shape[0] > 0
            else torch.arange(seq_len, device=hidden_states.device, dtype=torch.long)
        )
        try:
            causal_mask = create_causal_mask(
                config=self._inner_model.config,
