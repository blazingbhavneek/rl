...
class BaseModel:
    ...
    def _lm_head_logits_chunked(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if self.chunk_size is None:
            logits = self._postprocess_logits(lm_head(hidden_states))
            # logits: [B, T, V]
            return logits
        ...
        for start in range(0, seq_len, chunk):
            ...
            # h_slice: [B, t_chunk, H]
            logits_part = self._postprocess_logits(lm_head(h_slice))
    ...
    def _token_logprobs_chunked(
        ...
        for start in range(0, completion_len, chunk):
            ...
            token_slice = token_ids[:, start:end].to(
                hidden_comp.device, non_blocking=True
            )
    ...
    def backward(
        ...
        completion_token_lists = [
            self.tokenizer(text, add_special_tokens=False, return_attention_mask=False)[
                "input_ids"
            ]
        ...
        with torch.enable_grad():
            ...
            mean_logp = None
            batch_logprobs = None
            # Only materialize full token logprobs when the chosen loss path needs them.
            # The masked-mean streaming reduction computes its own chunked logprobs
            # directly from hidden_comp, which keeps peak memory tied to chunk size.
            if batch_loss_callable is not None:
                ...
                if stream_mode == "masked_mean_logprob":
                    ...
                    batch_loss = batch_loss_callable(
                        batch_logprobs, completion_mask, hidden_comp
                    )
